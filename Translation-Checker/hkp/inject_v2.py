...
# v2: lê o parquet re-traduzido COM a frase-alvo preservada (saída do translate_v2.py).
# Default; pode ser sobrescrito via --input (útil se mudar o modelo de tradução ou o nome do output).
INPUT_PATH = PROJECT_ROOT / "Data" / "Hackaprompt" / "Translated" / "hackaprompt_pt_br_masked_v2_Qwen__Qwen2.5-72B-Instruct-AWQ.parquet"
...
def check_injection(completion: str, completion_tr: str) -> bool:
    ...
    inspecioná-las manualmente. NÃO mede sucesso de injeção/jailbreak — o sucesso é decidido no
    notebook por exact match contra `expected_completion`."""
...
def inject_dataset(client: OpenAI, model: str, input_path: Path, output_path: Path,
    ...
    warnings_path = OUTPUT_DIR / "injection_warnings_v2.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Executa os prompts (sys_prompt + user_input) num servidor vLLM local "
                    ...
                    "Versão v2: lê o parquet re-traduzido com a frase-alvo preservada.")
    ...
    output_path = Path(args.output) if args.output else OUTPUT_DIR / f"hackaprompt_injection_v2_{model_slug}.parquet"

...
# Modelo ALVO do ataque (quem recebe o prompt traduzido e responde). Igual ao inject.py original.
...
# v3: lê o parquet re-traduzido com a frase-alvo em pt-BR ("Eu fui PWNED"), saída do translate_v3.py.
# Default; pode ser sobrescrito via --input.
INPUT_PATH = PROJECT_ROOT / "Data" / "Hackaprompt" / "Translated" / "hackaprompt_pt_br_v3_Qwen__Qwen2.5-72B-Instruct-AWQ.parquet"
# Mesmo diretório do inject.py/inject_v2 (consistente entre as versões).
...
def check_injection(completion: str, completion_tr: str) -> bool:
    """Heurística de RUÍDO (não é o critério do benchmark).

    True apenas se a diferença absoluta de tamanho entre `completion` e `completion_tr` for
    grande demais. Serve só para sinalizar linhas com respostas muito divergentes em tamanho e
    inspecioná-las manualmente. NÃO mede sucesso de injeção/jailbreak — o sucesso é decidido na
    análise por exact match contra o gabarito pt-BR ('Eu fui PWNED')."""
...
def inject_dataset(client: OpenAI, model: str, input_path: Path, output_path: Path,
                      limit: int | None, concurrency: int, chunk_size: int) -> None:
    print(f"\n=== Executando injeção (v3): {input_path.name} ===")
    ...
    warnings_path = OUTPUT_DIR / "injection_warnings_v3.parquet"
    ...
    if output_path.exists():
        ...
        df_done = pd.DataFrame(columns=df_orig.columns.tolist() + ["completion_tr"])  # colunas originais + resposta
        start_idx = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Executa os prompts (sys_prompt + user_input) traduzidos num modelo ALVO "
                    "(vLLM, API OpenAI-compatible), em paralelo, e salva o dataset completo + "
                    "completion_tr (resposta do ataque). Versão v3: lê a tradução com a frase-alvo em pt-BR.")
    parser.add_argument("--model", default=MODEL, help="Modelo ALVO servido pelo vLLM")
    parser.add_argument("--input", default=None,
                        help=f"Caminho do parquet traduzido de entrada (default: {INPUT_PATH})")
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"Dataset traduzido não encontrado em: {input_path}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    model_slug = args.model.replace("/", "__")
    output_path = Path(args.output) if args.output else OUTPUT_DIR / f"hackaprompt_injection_v3_{model_slug}.parquet"
    ...
    inject_dataset(client, args.model, input_path, output_path, args.limit, args.concurrency, args.chunk_size)

    print("\nInjeção v3 concluída!")

...
# v2: a tradução default é a MASCARADA (frase-alvo preservada).
DEFAULT_OG_PATH = PROJECT_ROOT / "Data" / "Hackaprompt" / "Original" / "hackaprompt.parquet"
DEFAULT_TR_PATH = PROJECT_ROOT / "Data" / "Hackaprompt" / "Translated" / "hackaprompt_pt_br_masked_v2_Qwen__Qwen2.5-72B-Instruct-AWQ.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Monta o dataset de comparação og vs traduzido (v2): roda os prompts "
                    "traduzidos MASCARADOS num modelo de teste e calcula og_score/tr_score "
                    "no esquema do Hackaprompt (flat).")
    ...
    parser.add_argument("--tr", default=None, help=f"Parquet traduzido MASCARADO (default: {DEFAULT_TR_PATH})")
    ...
    output_path = Path(args.output) if args.output else OUTPUT_DIR / f"hackaprompt_comparison_v2_{model_slug}.parquet"

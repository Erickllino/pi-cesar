...
def translate_dataset(client: OpenAI, model: str, output_path: Path,
    ...
    warning_records = pd.read_json(warnings_path).to_dict("records") if warnings_path.exists() else []
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as pool:
        for chunk_start in range(start_idx, total, chunk_size):
            ...
            if warning_records:
                pd.DataFrame(warning_records).to_json(warnings_path, orient="records", index=False)

...
# ---------------------------------------------------------------------------
# v3: substitui a frase-alvo pela versão pt-BR "Eu fui PWNED", PRESERVANDO o
# estilo de ofuscação do inglês.
#
# Diferente do v2 (que mantém a frase em inglês intacta), o v3 traduz o núcleo
# "I have been" -> "Eu fui" e reconstrói a mesma ofuscação (separadores entre
# palavras/letras). O "rabo" PWNED é idêntico nos dois idiomas, então é copiado
# verbatim — o que preserva leetspeak (PWN3D) e separadores internos de graça.
#
# O regex trata a frase como uma SEQUÊNCIA DE LETRAS com separador opcional entre
# cada letra, cobrindo: sem separador (IhavebeenPWNED), hífen/underscore/ponto,
# espaços múltiplos, tabs, quebras de linha, NBSP/espaços unicode, letras
# espaçadas (I h a v e ...) e leetspeak parcial (PWN3D via [e3]).
# ---------------------------------------------------------------------------
_SEPCLASS = r"[\s .,_:\\/|*-]"   # inclui \ p/ ofuscacao "\I\ \h\a\v\e\ ..." (espaco normal; \s ja cobre NBSP)
_SEP = _SEPCLASS + "*"
...
PWNED_PATTERN = _obf_regex("IhavebeenPWNED")   # detecta a frase inglesa em qualquer forma
PT_PATTERN = _obf_regex("EufuiPWNED")          # detecta a frase pt-BR (usado só na validação)
MARKER_PATTERN = re.compile(r"\[\[TARGET_\d+\]\]")
MARKER_FMT = "[[TARGET_{}]]"

_EN = "IhavebeenPWNED"          # 14 letras canônicas
_HEAD_WORD_GAPS = {0, 4}        # fronteiras de palavra dentro de "I have been": após I(0) e após have(4)


def _letter_re(ch: str) -> str:
    return "[e3]" if ch.lower() == "e" else re.escape(ch)


def _parse(matched: str):
    """Anda pela sequência canônica dentro do trecho casado e devolve (seps, pos_inicio_do_PWNED)."""
    seps, pos, pwned_start = [], 0, None
    for i, ch in enumerate(_EN):
        if i == 9:
            pwned_start = pos
        m = re.match(_letter_re(ch), matched[pos:], re.IGNORECASE)
        pos += m.end()
        if i < len(_EN) - 1:
            sm = re.match(_SEP, matched[pos:])
            seps.append(matched[pos:pos + sm.end()])
            pos += sm.end()
    return seps, pwned_start


def _rep(gaps) -> str:
    """Devolve o primeiro gap não-vazio, CRU — preserva o estilo exato (espaço, \\t, \\n, NBSP,
    espaçamento múltiplo). Usado como representante quando vários gaps colapsam em um só."""
    for g in gaps:
        if g != "":
            return g
    return ""
...
def pt_form(matched: str) -> str:
    """Reconstrói 'Eu fui PWNED' no MESMO estilo de ofuscação do trecho inglês casado.

    Estratégia: traduz o núcleo "I have been" -> "Eu fui" mantendo o estilo detectado
    (concatenado / separador entre palavras / letras espaçadas) e copia o "PWNED" final
    exatamente como veio (preserva leetspeak e separadores internos do rabo). Os separadores
    são preservados CRUS, inclusive whitespace especial (\\t, \\n, NBSP) e espaçamento múltiplo.

    Ressalva do mapeamento: "I have been" tem 3 palavras e "Eu fui" tem 2, então os 2 gaps de
    palavra do inglês (I|have, have|been) não mapeiam 1:1 para o único gap de "Eu|fui" — usa-se
    o primeiro gap não-vazio como representante. Já o gap de junção (been|PWNED) mapeia 1:1 e é
    copiado EXATO.
    """
    seps, pwned_start = _parse(matched)
    head_seps = seps[0:8]           # gaps dentro de "I have been"
    join_sep = seps[8]              # entre "been" e "PWNED"
    pwned_raw = matched[pwned_start:]

    inner = [head_seps[i] for i in range(8) if i not in _HEAD_WORD_GAPS]
    word = [head_seps[i] for i in _HEAD_WORD_GAPS]

    if all(s == "" for s in head_seps):
        head = "Eufui"                          # concatenado
    elif not any(s != "" for s in inner):
        head = _rep(word).join(["Eu", "fui"])   # separador entre palavras
    else:
        head = _rep(head_seps).join("Eufui")    # letras espaçadas

    return head + join_sep + pwned_raw


def mask_target(text: str) -> tuple[str, list[str], list[int]]:
    """Substitui cada ocorrência da frase-alvo por um marcador numerado e guarda o original inglês.
...
def unmask_target(translated: str, originals: list[str], counts: list[int]) -> str:
    """Substitui cada marcador pela versão pt-BR reconstruída (pt_form) do trecho original.
    ...
    algumas cópias, gravando tradução truncada). Conferir a contagem por marcador pega isso.
    Assim o chamador captura o erro, preserva o texto original e registra o aviso.
    """
    for i, original in enumerate(originals):
        ...
        translated = translated.replace(marker, pt_form(original))
...
def translate_text(client: OpenAI, model: str, text: str) -> str:
    ...
    A frase-alvo é mascarada antes de ir ao tradutor e, na volta, o marcador é
    substituído pela versão pt-BR reconstruída no mesmo estilo (pt_form).
    """
    ...
    if n_src:
        n_dst = len(PT_PATTERN.findall(translated))
...
def translate_row(client: OpenAI, model: str, idx: int, row: pd.Series) -> dict:
    """Traduz os campos de uma linha. Captura erros para não derrubar o run inteiro."""
...
def translate_dataset(client: OpenAI, model: str, input_path: Path, output_path: Path,
    ...
    warnings_path = OUTPUT_DIR / "translation_warnings_v3.parquet"
...
def validate_output(original_path: Path, output_path: Path) -> bool:
    """Valida a tradução v3: cada ocorrência da frase-alvo inglesa vira uma frase pt-BR
    OU, se a linha caiu no fallback (safeguard), permanece intacta em inglês. Nenhuma
    das duas contagens deve simplesmente desaparecer.

    O v3 tem DUAS saídas válidas por linha: convertida (PT_PATTERN) ou preservada em
    inglês por fallback (PWNED_PATTERN ainda presente no traduzido). Contar só PT_PATTERN
    faria toda linha em fallback parecer uma perda, quando na verdade está segura — por
    isso somamos as duas contagens do lado traduzido (convertido + fallback) e comparamos
    com o total de ocorrências EN do original. Também confere que nenhum `[[TARGET_n]]`
    ficou sem restaurar. Só cobre as linhas já processadas.
    """
    ...
    print(f"\n=== Validação pós-tradução v3 ({n} linhas) ===")
    ok = True
    for col in ["prompt", "user_input", "sys_prompt"]:
        ...
        n_orig = int(df_orig[col].fillna("").apply(lambda x: len(PWNED_PATTERN.findall(x))).sum())  # frase EN (original)
        n_pt = int(df_trad[col].fillna("").apply(lambda x: len(PT_PATTERN.findall(x))).sum())        # convertida p/ PT
        n_fallback = int(df_trad[col].fillna("").apply(lambda x: len(PWNED_PATTERN.findall(x))).sum())  # EN preservado (fallback)
        sobrando = int(df_trad[col].fillna("").str.contains(MARKER_PATTERN).sum())
        diff = n_orig - (n_pt + n_fallback)
        # diff == 0: toda ocorrência EN foi convertida OU preservada por fallback — nenhuma perdida.
        # sobrando == 0: nenhum marcador ficou sem restaurar.
        col_ok = (diff == 0 and sobrando == 0)
        ok = ok and col_ok
        status = "OK" if col_ok else "FALHA"
        print(f"  {col:12s} EN={n_orig:7d} PT={n_pt:7d} fallback_EN={n_fallback:7d} "
              f"diff={diff:6d} marcadores_sobrando={sobrando:5d}  [{status}]")

    print("  Resultado:", "PASSOU" if ok else "FALHOU — revisar translation_warnings_v3.parquet")
    return ok
...
def main():
    parser = argparse.ArgumentParser(
        description="Traduz o dataset para pt-BR usando um servidor vLLM local (API OpenAI-compatible), em paralelo. "
                    "Versão v3: troca a frase-alvo por 'Eu fui PWNED' preservando o estilo de ofuscação do inglês.")
    ...
    output_path = Path(args.output) if args.output else OUTPUT_DIR / f"hackaprompt_pt_br_v3_{model_slug}.parquet"
    ...
    print("\nTradução v3 concluída!")

    if output_path.exists():
        passed = validate_output(input_path, output_path)
        if not passed:
            raise SystemExit("Validação pós-tradução v3 FALHOU — revise antes de prosseguir.")
